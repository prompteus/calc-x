...
for dset_name, dset in preprocessed_datasets.items():
    ...
    for dset_split, subdset in dset.items():
        preprocessed_datasets_labeled[dset_name][dset_split] = subdset.map(preprocess_fn).filter(lambda example: example['labels'] is not None)
...
metrics = MyBaselineMetrics(
    ...
    datasets_id_length={k:valid_size for k in dataset_to_keys.keys()}  # TODO: ordering and sizes must match eval_dataset
