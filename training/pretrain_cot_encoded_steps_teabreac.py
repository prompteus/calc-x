...
def flatten_sample_per_step(x: Dataset, question_key: str,
                            chain_key: str, answer_key: str, sep: str = ". ") -> Iterator[dict[str, List[str]]]:

    steps = [step.strip() for step in x[chain_key].split(sep)]
...
val_dataset = datasets.load_dataset("MU-NLPC/Calc-gsm8k", split="validation")
val_dataset = val_dataset.map(lambda row: {"question_text": row["question"],
                                           "answers_text": row["result"],
                                           "context_text": row["chain"]})

keys = {"question_key": "question_text", "answer_key": "answers_text", "chain_key": "context_text"}

dataset = DatasetDict({"train": Dataset.from_pandas(tea_train), "validation": val_dataset})
...
preprocessed_datasets["teabreac"] = dataset
...
metrics = gadgets.metrics.MyMetrics(
    ...
    datasets_id_length={"teabreac": len(preprocessed_datasets["teabreac"]['validation'])},
